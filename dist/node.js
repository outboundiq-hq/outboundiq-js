...
var require2 = module$1.createRequire((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('node.js', document.baseURI).href)));
var http = require2("http");
var https = require2("https");
var originalHttpRequest = http.request;
var originalHttpsRequest = https.request;
var originalHttpGet = http.get;
var originalHttpsGet = https.get;
var isPatched2 = false;
var userContextResolver2 = null;
function setUserContextResolver(resolver) {
  userContextResolver2 = resolver;
}
...
function wrapRequest(originalFn, protocol) {
  return function wrappedRequest(urlOrOptions, optionsOrCallback, maybeCallback) {
    const startTime = performance.now();
    let options;
    let url;
    ...
    if (url.includes("outboundiq.dev") || url.includes("outboundiq.io") || url.includes("outboundiq.test") || url.includes("outboundiq.com")) {
      return originalFn.apply(this, [urlOrOptions, optionsOrCallback, maybeCallback]);
    }
    const userContext = userContextResolver2?.() ?? null;
    let requestBody = "";
    const req = originalFn.apply(
      this,
      [urlOrOptions, optionsOrCallback, maybeCallback]
    );
    const originalWrite = req.write.bind(req);
    req.write = function(chunk, encodingOrCallback, maybeCallback2) {
      if (chunk) {
        requestBody += chunk.toString().substring(0, 5e3);
      }
      return originalWrite(chunk, encodingOrCallback, maybeCallback2);
    };
    req.on("response", (res) => {
      const duration = performance.now() - startTime;
      let responseBody = "";
...
function wrapGet(_originalFn, wrappedRequest) {
  return function wrappedGet(urlOrOptions, optionsOrCallback, maybeCallback) {
...
// src/node.ts
var require3 = module$1.createRequire((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('node.js', document.baseURI).href)));
...
exports.patchNodeHttp = patchNodeHttp;
...
exports.setUserContextResolver = setUserContextResolver;
exports.shouldIgnore = shouldIgnore;
exports.shutdown = shutdown;
exports.track = track;
exports.unpatchFetch = unpatchFetch;
exports.unpatchNodeHttp = unpatchNodeHttp;
